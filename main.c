...
/*   Updated: 2023/08/05 14:35:19 by egervais         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	child(t_pipe *pip, char **envp, char **av)
{
	...
	if (!pip->cmdone)
		free2(pip->cmd1, pip->cmdone, pip, "invalid command\n");
	if (execve(pip->cmdone, pip->cmd1, envp) == -1)
		free2(pip->cmd1, pip->cmdone, pip, "execution failed\n");
}
...
static void	parent(t_pipe *pip, char **envp, char **av)
{
	...
	if (!pip->cmdtwo)
		free2(pip->cmd2, pip->cmdtwo, pip, "invalid command\n");
	if (execve(pip->cmdtwo, pip->cmd2, envp) == -1)
		free2(pip->cmd2, pip->cmdtwo, pip, "execution failed\n");
}
...
int	main(int ac, char **av, char **envp)
{
	...
	if (!pip || init_struct(pip, av, envp, ac) || pipe(pip->p) == -1)
		freee2(pip);
	pip->pid = fork();
	if (pip->pid == -1)
	{
